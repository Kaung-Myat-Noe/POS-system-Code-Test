...
namespace pos.sys.Controllers
{
    ...
    //[Authorize]
    public class Transaction_Controller : BaseController
    {
        internal readonly ICouponRepository _coupon;
        internal readonly ILogger<Transaction_Controller> _logger;
        private readonly AppSettings _settings;
        HttpStatusCode code;
        public Transaction_Controller(ICouponRepository coupon, ILogger<Transaction_Controller> logger, IOptionsMonitor<AppSettings> settings)
        {
            _coupon = coupon;
            _logger = logger;
        ...
        [HttpPost("CouponCheck")]
        public IActionResult CouponCheck(string code)
        {
            AssignLogID();
            ...
            try
            {
                coupon Coupon = _coupon.Query(x=>x.code == code && x.quantity >0).FirstOrDefault();
                if (Coupon != null)
                {
                    responseModel.Data = SuccessCode.ValidCoupon;
                    return Ok(new { RefNo = RefNo, Data = responseModel.Data, Error = responseModel.Error });
                }
                else
                {
                    responseModel.Data = SuccessCode.InValidCoupon;
